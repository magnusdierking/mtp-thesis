...
def run_interactive(  # noqa: PLR0912, PLR0915
        ...
        while viewer.is_running():
            ...
            for i in range(sim_steps_per_replan):
                ...
                if data_log_path is not None:
                    error_log.append(np.linalg.norm(controller.task._get_position_err(mj_data) 
    ...
    if data_log_path is not None:
        path = data_log_path + "/error_log_seed_" + str(seed) + ".npy"
        np.save(path, np.array(error_log))
        print(f"\nError log saved to {path}")
        # same for beta
        if hasattr(policy_params, 'beta'):
            path = data_log_path + "/beta_log_seed_" + str(seed) + ".npy"
            np.save(path, np.array(beta_log))
            print(f"\nBeta log saved to {path}")
