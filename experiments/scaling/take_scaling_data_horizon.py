...
# horizons = [4, 6]
# steps_per_control = [4, 6]


path = Path('./scaling_data_horizon/')
path.mkdir(parents=True, exist_ok=True)
path = path / f"{args.algorithm}.pkl"

# -1 because initial trime might be not counted
data = np.empty((len(horizons), len(steps_per_control), max_step-1))

for s, horizon in enumerate(horizons):
    for j, steps in enumerate(steps_per_control):
        print(f"\nRunning experiment with horizon: {horizon}, steps_per_control: {steps}")
        ...
        task = PushTFranka(ik_type = 'pinv',
                            ...
                            sim_steps_per_control_step=steps,
