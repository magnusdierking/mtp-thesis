...
def run_interactive(  # noqa: PLR0912, PLR0915
    ...
    with mujoco.viewer.launch_passive(
        mj_model, mj_data, show_left_ui=show_ui, show_right_ui=show_ui
    ) as viewer:
        ...
        if show_traces:
            ...
            for i in range(
                ...
                * controller.task.planning_horizon
            ):
                mujoco.mjv_initGeom(
        ...
        while viewer.is_running():
            ...
            sites_of_interest = policy_params.predicted_state[
                :, -1, ...
            ]  # (domains, prediction_horizon, sites, 7)
            # print("Sites of interest shape:", sites_of_interest.shape)
            # true poses
            new_observation1 = jnp.concatenate(
                (
                    jnp.array(mj_data.site_xpos[site_id1]),
                    jnp.array(mat2quat(mj_data.site_xmat[site_id1])),
                ),
                axis=-1,
            )
            new_observation2 = jnp.concatenate(
                (
                    jnp.array(mj_data.site_xpos[site_id2]),
                    jnp.array(mat2quat(mj_data.site_xmat[site_id2])),
                ),
                axis=-1,
            )
            new_observation3 = jnp.concatenate(
                (
                    jnp.array(mj_data.site_xpos[site_id3]),
                    jnp.array(mat2quat(mj_data.site_xmat[site_id3])),
                ),
                axis=-1,
            )
            new_observation4 = jnp.concatenate(
                (
                    jnp.array(mj_data.site_xpos[site_id4]),
                    jnp.array(mat2quat(mj_data.site_xmat[site_id4])),
                ),
                axis=-1,
            )
            ...
            if np.allclose(np.array(new_observation1), old_observation1, atol=1e-2) and np.allclose(
                ...
                alphas = (
                    ...
                    * (distances - np.min(distances))
                    / (np.max(distances) - np.min(distances) + 1e-12)
                )
                        ...
                        for d, color in enumerate(colors):  # num_randomizations
                            for j in range(controller.task.planning_horizon):
                                ...
                                mujoco.mjv_connector(
                                    ...
                                    trace_width,
                                    rollouts.trace_sites[
                                        d, i, j, k, :3
                                    ],  # ! randomizations x rollouts x horizon x sites
            ...
            if hasattr(controller, "beta"):
                print(
                    f"Realtime rate: {rtr:.2f}, \
                      plan time: {plan_time:.4f}s, \
                      sim time: {mj_data.time:.2f}s, \
                      beta: {controller.beta:.3f}",
                    end="\r",
                )
            else:
                print(
