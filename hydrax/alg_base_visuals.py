...
class Trajectory:
    ...
    state_bins: jax.Array   # Placeholder for state bins
    ...
    def eval_rollouts(
        ...
        def _scan_fn(
            ...
            def _fill_bins(state_bins, state):
                ...
                return state_bins
            state_bins = _fill_bins(state_bins, x)
            # jax.debug.print("After running cost and trace sites")

            def _step_visual(_, x, state_bins):
                state_bins = _fill_bins(state_bins, x)
            ...
            x = jax.lax.fori_loop(
                ...
                lambda _, x: _step_visual(_, x, state_bins),
                x.replace(ctrl=u_mapped),
            )
            # jax.debug.print("After mjx.step")
            return x, (x, cost, sites, state_bins)
        ...
        final_state, (states, costs, trace_sites, state_bins) = jax.lax.scan(
        ...
        # TODO fill state_bins with the final state
