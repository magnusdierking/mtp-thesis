...
def interpolate_path(path: jax.Array, num_points: int) -> jax.Array:
    ...
    return jax.vmap(linspace, in_axes=(0, 0, None))(start, goal, num_points).reshape(
        -1, path.shape[-1]
    )
...
class MTP(SamplingBasedController):
    ...
    def __init__(
        ...
        if keep_elites > num_elites:
            print(
                f"Warning: keep_elites ({keep_elites}) > num_elites ({num_elites}). Setting keep_elites = num_elites."
            )
        ...
        self.bknots = self.start_clamped_knot_vector(
            (self.M + 1), self.degree, dtype=control_dtype
        )
        ...
        if self.use_spline:
            self.b_knots_mppi = self.start_clamped_knot_vector(
                self.task.planning_horizon, self.degree, dtype=control_dtype
            )
        ...
        if savgol_filter:
            self.savgol_filter_fn = make_savgol_filter(
                window_length=7, polyorder=2, axis=1
            )
    ...
    def start_clamped_knot_vector(self, num_ctrl_points, degree, dtype=jnp.float32):
        ...
        rest = (jnp.arange(1, nbr_rest, dtype=dtype)) * (
            1.0 / jnp.array(nbr_rest, dtype=dtype)
        )
    ...
    def init_params(self, seed: int = 0) -> MTPParams:
        ...
        mean = (
            jnp.zeros((self.task.planning_horizon, self.task.nu))
            + self.sigma_start * noise
        )
        spline = mean.copy()
        elites = spline[None, ...].repeat(self.keep_elites, axis=0)
        cov = jnp.full_like(mean, self.sigma_start)
        predicted_state = jnp.zeros(
            (
        ...
        #                              7), dtype=jnp.float32)
        domain_weights = (
            jnp.ones((self.num_randomizations,), dtype=jnp.float32)
            / self.num_randomizations
        )
    ...
    def sample_controls(self, params: MTPParams) -> Tuple[jax.Array, MTPParams]:
        ...
        if self.nbr_mtp_samples > 1:
            ...
            layer_indices = jax.random.randint(
                sample_rng, (self.nbr_mtp_samples, self.M), 0, self.N - 1
            )
            ...
            # !! Double Check
            init_sample = jnp.repeat(
                params.spline[self.last_a_idx][None, None, :],
                self.nbr_mtp_samples,
                axis=0,
            )

            control_points = jnp.concatenate([init_sample, control_points], axis=1)
            ...
            remain = T - num_interp * (
                self.M
            )  # tail we fill with the last control point
            ...
            if self.interpolation == "akima":
                ...
                mtp_controls_full = jnp.empty(
                    (control_points.shape[0], T, U), dtype=interp_vals.dtype
                )
                mtp_controls_full = mtp_controls_full.at[:, :interp_len].set(
                    interp_vals
                )
                ...
                mtp_controls_full = jnp.einsum(
                    "...md,hm->...hd", control_points, self.bmat
                )
            elif self.interpolation == "linear":
                # interp_vals: (B, interp_len, U)
                interp_vals = jax.vmap(interpolate_path, in_axes=(0, None))(
                    control_points, num_interp
                )
        ...
        if self.nbr_mppi_samples > 0:
            ...
            if self.use_spline:
                mppi_controls = jnp.einsum(
                    "...md,hm->...hd", mppi_controls, self.b_mat_mppi
                )
            elif self.savgol_filter:
                mppi_controls = self.savgol_filter_fn(mppi_controls)
            out = out.at[
                1 + self.nbr_mtp_samples : 1
                + self.nbr_mtp_samples
                + self.nbr_mppi_samples
            ].set(mppi_controls)
    ...
    def update_params(self, params: MTPParams, rollouts: Trajectory) -> MTPParams:
        ...
        if self.sample_weighting == "cem-softmax":
            ...
            weights = jnp.nan_to_num(
                jax.nn.softmax(-costs[elite_indices] / self.temperature, axis=0)
            )
        ...
        if self.update_cov:
            cov = jnp.sqrt(
                jnp.sum(weights[:, None, None] * (controls - mean) ** 2, axis=0)
            )
        ...
        # Rollouts trace sites is (domains, samples, control steps, sites, 7)
        print("Rollout trace sites :", rollouts.trace_sites.shape)
        predicted_state = rollouts.trace_sites[:, next_idx, : (self.last_a_idx), ...]
