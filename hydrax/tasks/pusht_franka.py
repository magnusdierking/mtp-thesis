...
class PushTFranka(Task):
    ...
    def __init__(
        ...
        if actuation_type == "position":
            mj_model = mujoco.MjModel.from_xml_path(
                (
                    get_root_path() / "models" / "fr3_pushT_pos" / "scene_mjx.xml"
                ).as_posix()
            )
        ...
        ghost_Ts = [f"ghost_block_{i}" for i in range(10)]
        self.mocap_T_bids = []
        ...
        self.block_joint_idxs = [
            mj_model.joint(name).id for name in self.block_joint_names
        ]
    ...
    def reset(self, seed: int = 0) -> None:
        ...
        pos_x = self.det_init.get(
            "block_pos_x", sign_x * np.random.uniform(low=0.1, high=0.25)
        )
        ...
        for i in range(max_iters):
            ...
            J = np.vstack(
                [J_pos[:, self.dof_adr], J_rot[:, self.dof_adr]]
            )  # shape (6, n_joints)
    ...
    def _get_orientation_err(self, state: mjx.Data) -> jax.Array:
        ...
        block_quat = jnp.where(
            block_quat[0] < 0, -block_quat, block_quat
        )  # ensure w >= 0
        ...
        axis_angle_error = mjx._src.math.quat_sub(
            block_quat, goal_quat
        )  # gives axis angle of relative rotation
    ...
    def make_differential_ik_mapper(self, type: str = "pinv", fk_jac=None):
        ...
        @jax.jit
        def diff_ik_mapper_transpose(
            data: mjx.Data, control_xy: jax.Array
        ) -> jax.Array:
            """
        ...
        def diff_ik_mapper_pinv(
            ...
            twist_cmd = jnp.concatenate(
                [control_xy, jnp.zeros(4)]
            )  # [vx, vy, 0, 0, 0, 0]
        ...
        def ik_mapper_pinv(
            ...
            desired_pose = SE3(
                wxyz_xyz=jnp.concatenate(
                    [goal_quat, jnp.array([desired_xy[0], desired_xy[1], 0.03])]
                )
            )
            ...
            def body_fn(_i, qpos, data):
                ...
                current_pose = SE3(
                    wxyz_xyz=jnp.concatenate(
                        [data.xquat[self.ee_body_id], data.xpos[self.ee_body_id]]
                    )
                )
                ...
                dq_full = (
                    jnp.zeros_like(qpos)
                    .at[jnp.array(self.actuator_joint_idxs)]
                    .set(dq_act)
                )
